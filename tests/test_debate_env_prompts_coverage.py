...
def test_selfplay_judge_final_instruction_names_decision_xml():
    """The live judge turn should not rely on generic field instructions alone."""
    prompts = resolve_prompts("selfplay")
    instruction = prompts.render_instruction(
        "judge",
        "final",
        build_context(
    ...
    assert "debater_a" in instruction
    assert "debater_b" in instruction
    assert "tie" in instruction
