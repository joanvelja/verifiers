...
_CURRENT_MEMBER_GENERATION: contextvars.ContextVar[MemberGenerationPlan | None] = (
    contextvars.ContextVar("_CURRENT_MEMBER_GENERATION", default=None)
)
...
class Environment(ABC):
    ...
    async def get_model_response(
        ...
        member_id = request_context.member_id if request_context else None
    ...
    async def init_state(
        ...
        state["member_generation"] = generation or _CURRENT_MEMBER_GENERATION.get()
    ...
    @staticmethod
    def _coerce_member_generation_plan(
        raw: object,
    ) -> MemberGenerationPlan | None:
        if raw is None:
            return None
    ...
    def _generation_target_for_member(
        ...
        if member_id is None:
            return None
        plan = self._coerce_member_generation_plan(state.get("member_generation"))
        if plan is None:
            return None
        if plan is not state.get("member_generation"):
    ...
    def _generation_metadata(target: GenerationTarget) -> dict[str, Any]:
        ...
        return {
            "client": {
                ...
                "tool_parser": client.tool_parser,
                "reasoning_parser": client.reasoning_parser,
            },
            "model": target.model,
    ...
    def _group_generations(
        ...
        if len(generation) != group_size:
            raise ValueError(
                f"run_group generation list length {len(generation)} does not match "
    ...
    def _with_rollout_ids(inputs: list[RolloutInput]) -> list[RolloutInput]:
        ...
        for input in inputs:
            ...
            counts[example_id] += 1
            rollout_input = dict(input)
            rollout_input["rollout_id"] = (
                f"{canonical_example_id(example_id)}:{rollout_idx}"
            )
            with_ids.append(cast(RolloutInput, rollout_input))
        return with_ids
    ...
    def _generation_needs_rollout_identity(
        ...
        keys = {
            (
                "__none__"
                if generation is None
                else json.dumps(generation.model_dump(mode="json"), sort_keys=True)
            )
    ...
    def _filter_resume_items(
        ...
        if outputs and all(output.get("rollout_id") for output in outputs):
            ...
            filtered: list[MemberGenerationPlan | None] = []
            for input, plan in zip(inputs, generations, strict=True):
                if str(input["rollout_id"]) in completed:
                    continue
                filtered_inputs.append(input)
                filtered.append(plan)
            return filtered_inputs, filtered

        if outputs and Environment._generation_needs_rollout_identity(generations):
            raise ValueError(
                "Cannot resume list-valued generation plans from results without "
        ...
        for input, plan in zip(inputs, generations, strict=True):
            items_by_example_id[input["example_id"]].append((input, plan))
        for output in outputs:
            outputs_by_example_id[output["example_id"]].append(output)

        filtered_inputs: list[RolloutInput] = []
        filtered: list[MemberGenerationPlan | None] = []
        for example_id, example_items in items_by_example_id.items():
            completed = len(outputs_by_example_id[example_id])
            remaining = example_items[completed:rollouts_per_example]
            filtered_inputs.extend(input for input, _ in remaining)
            filtered.extend(plan for _, plan in remaining)
        return filtered_inputs, filtered
    ...
    def _generation_for_group(
        ...
        concrete = [plan for plan in generations if plan is not None]
        if not concrete:
            return None
        if len(concrete) != len(generations):
            raise ValueError("run_group generation plans must be all-or-none")
        first = concrete[0]
        if all(plan == first for plan in concrete):
            return first
        return concrete
    ...
    async def _run_group_states(
        ...
        async def run_one(
            input: RolloutInput, plan: MemberGenerationPlan | None
        ) -> State:
        ...
        rollout_tasks = [
            run_one(input, plan) for input, plan in zip(group_inputs, generations)
        ]
    ...
    async def generate(
        ...
        try:
            ...
            if results_path is not None and is_valid_eval_results_path(results_path):
                ...
                filtered_inputs, filtered_generations = self._filter_resume_items(
