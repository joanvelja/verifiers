...
- **Add a new multi-agent env.** Subclass `MultiAgentEnv`, implement `build_prompt(state, member_id, slot)` (respect the monotonic invariant) and `render_completion(state)`. Optional overrides: `extract_fields`, `visibility_policy`, `get_agent_binding`, `_build_probe_state`. Reference: `verifiers/envs/debate_env.py`.
...
2. `get_agent_binding("debater_a", state)` returns `(client, model)` for this member. Two sources: when `agent_bindings_fn` is set at `__init__`, the function is called on the current `state` to produce a per-episode bindings map (used for external-opponent training — the learner seat binds to `(None, None)` so the rollout-default client/model is used, and the opposite/judge seats bind to fixed external clients); otherwise falls back to the static `self.agent_bindings` dict. Both yield `(None, None)` for members with no binding (use the rollout-default client/model). Self-play setups use the static dict; per-episode seat assignment uses the function.
