...
- **`openai_chat_completions_token`** (TITO, *token-in*): server-side templating, but returns prompt and completion token IDs alongside text so the trainer doesn't re-tokenize. Use as the broad-coverage fallback when you trust the server's chat template to be stable across turns.
...
For multi-agent and debate training, prefer `renderer` when your rollout endpoint supports `/v1/generate` and your model has a supported renderer; environments such as `gpqa-debate` can set this as their package default. Use `openai_chat_completions_token` explicitly when you need broader model or endpoint coverage. See [reference § Built-in Clients](reference.md#built-in-client-implementations) for the full list.
