...
- **`openai_chat_completions_token`** (TITO) — server-side templating, returns token IDs alongside text. The trainer doesn't re-tokenize. Use as the broad-coverage fallback when the server's chat template is stable across turns.
- **`renderer`** — client-side tokenization via a per-model renderer in the [`renderers` package](https://github.com/PrimeIntellect-ai/verifiers/tree/main/packages/renderers). Install it with `uv add "verifiers[renderers]"` before using `client_type="renderer"`. Stronger token-preservation: `bridge_to_next_turn` keeps multi-turn rollouts merged into one sample and survives mid-completion truncation cleanly.

For multi-agent and debate training, prefer `renderer` when your endpoint supports `/v1/generate` and your model has a supported renderer. Use `openai_chat_completions_token` explicitly when you need broader endpoint or model coverage. See [Inference Client Types](training.md#inference-client-types) for the full breakdown.
